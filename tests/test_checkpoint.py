...
def test_checkpoint_splunk_ips(
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netops host="{{ host }}" sourcetype="cp_log" source="cp:security"'
    )
    search = st.render(
...
def test_checkpoint_splunk_web_api(
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netops host="{{ host }}" sourcetype="cp_log" source="cp:audit"'
    )
    search = st.render(
        epoch=epoch, bsd=bsd, host=host, date=date, time=time, tzoffset=tzoffset
    )
...
security_product_events_with_originsicname = [
    ...
    'time={{ epoch }}|hostname={{ host }}|severity=Informational|product=IPS|action=Drop|ifdir=inbound|ifname=eth0|loguid={0x5d31915e,0x6d33e67b,0xd6c2bda1,0x880b3b39}|origin=10.160.99.101|originsicname=cn\=cp_mgmt,o\=gw-02bd87..4zrt7d|time=1608947981|attack=Streaming Engine: TCP SYN Modified Retransmission|dst=4.4.4.4',   
...
@pytest.mark.parametrize("event", network_product_dummy_events)
